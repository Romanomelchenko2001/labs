...
long long Great_Com_Div(long long a, long long b)
{
    while (b != 0)
    {
        long long r = a % b;
...
long long searchforx(long long* equations, long long N, long long& modulus)
{
    long long modulo = 1;
...
bool check(long long* equations, long long N)
{
    for (long long i = 0; i < N - 1; i++)
        for (long long j = i + 1; j < N; j++)
            if (Great_Com_Div(equations[3 * j + 2], equations[3 * i + 2]) != 1) return false;
...
void main(int argc, char* argv[])
{
    ...
    x = searchforx(eqs, N_proc, modulus);
    ...
    }if (ProcRank == 0)
    {
        ...
        if (!check(equations, N))
            MPI_Abort(MPI_COMM_WORLD, -1);
        x_ = searchforx(eq_f, ProcNum, mod_);
